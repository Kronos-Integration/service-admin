...
export function sshServer(service, config) {
  ...
  const server = new Server(config, client => {
    ...
    client
            ...
            .once('shell', (accept, reject) => {
              ...
              const commands = {
                ...
                services() {
                  ...
                  function pa(object, ca, prefix) {
                    for (const an in ca) {
                      const c = ca[an];
                      const value = object[an];
                      const cd = c.description
                        ? ' (' + c.description + ')'
                        : '';

                      if (c.attributes) {
                        output.log(`    ${prefix}${an}: ${cd}`);
                        pa(value, c.attributes, `${an}.`);
                      } else {
                        output.log(`    ${prefix}${an}: ${value} ${cd}`);
                      }
                    }
                  }

                  Object.keys(manager.services).forEach(n => {
                    const service = manager.services[n];
                    output.log(`  ${n}:`);
                    output.log(`    state: ${service.state}`);
                    pa(service, service.configurationAttributes, '');
                  });
                },
                flows() {
                  output.log(`flows:`);
                  Object.keys(manager.flows).forEach(n => {
                    const flow = manager.flows[n];
                    output.log(`  ${n}:`);
                    output.log(`    state: ${flow.state}`);
                    output.log(`    logLevel: ${flow.logLevel}`);
                  });
                },
                interceptors() {
                ...
                loglevel(args) {
                  ...
                  output.log(
                    `logLevel of ${service.name} set to ${service.logLevel}`
                  );
                },
                start(args) {
      ...
      .on('end', () => {})
      .on('error', err => {});
  }).listen(config.port || 0, () => {
    service.info({
      message: 'ssh server listening',
