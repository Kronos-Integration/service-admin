...
exports.initialize = function (manager) {
	...
	return { * list(next) {
			...
			* fetch(id, next) {
				...
				manager.flowDefinitions[id].start().then(function () {
					// TODO what to do with the returned promise ?
					console.log(`flow has been started: ${id}`);
				});
			}, * stop(id, next) {
				manager.flowDefinitions[id].stop().then(function () {
					// TODO what to do with the returned promise ?
				...
				try {
					...
					const result = {};
					this.body = result;
					yield manager.registerFlow(json, sr.createReporter(kronosStep.ScopeDefinitions, function (reporter) {
