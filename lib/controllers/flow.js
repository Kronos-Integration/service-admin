	...
	return { * list(next) {
			...
			* fetch(id, next) {
				...
				manager.deleteFlow(id).then(function () {
					...
					this.body = `flow has been deleted: ${id}`;
				}, function (reject) {
					this.body = reject;
				});
				this.body = {};
				...
				try {
					...
					//sr.adapter = scopeReporter.nullAdapter;

					yield manager.registerFlow(manager.getStepInstance(json), sr);
				} catch (e) {
					this.throw(e, 500);
				}
