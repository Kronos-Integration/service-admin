...
exports.manager = function (manager, options) {
  return manager.then(function (manager) {
    ...
    app.use(route.post('/flows/:id/pause', fc.pause));
    ...
    */
          ...
          return Promise.all([protoShutdown(), new Promise(function (resolve, reject) {
            m.httpServer.close(function (error) {
              if (error) {
                ...
                delete m.app;
                delete m.httpServer;
                // TODO unregister kronos+http: endpoint scheme
                resolve(manager);
              }
            });
          })]);
    ...
    manager.registerEndpointScheme("kronos+http", require('./endpoints/kronos-http'));
    const ec = require('./controllers/endpoints').initialize(manager);

    //manager.controller.endpoints = ec;

    app.use(route.post('/endpoints', ec.list));
