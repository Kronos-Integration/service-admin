...
exports.manager = function (options) {
  ...
  return kronos.manager(options).then(function (manager) {
    ...
    manager.app.use(route.get(healthMountPoint, healthController.status));
    manager.app.use(route.get('/flows', flowController.list));
    manager.app.use(route.get('/flows/:id', flowController.fetch));
