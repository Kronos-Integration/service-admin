...
function registerDonkeyNodeCheck(manager, cb) {
	consul.agent.check.list(function (err, result) {
		...
		if (myCheck) {
			...
			console.log(
				`${manager.consul.service.kronos.check.name} already defined (${myCheck.Status}): ${JSON.stringify(myCheck)}`
			);

			cb(undefined);
		} else {
			consul.agent.check.register(manager.consul.service.kronos.check, cb);
		}

		manager.consul.checkerInterval = setInterval(function () {
			consul.agent.check.pass(manager.consul.service.kronos.check.name, function (
				err) {
				if (err) {
					return;
				}
	...
	consul.agent.service.list(function (err, result) {
		...
		if (myService) {
			...
			consul.agent.service.register({
				...
				"serviceid": manager.consul.services.kronos.id,
				"notes": "kronos node",
				"port": port,
