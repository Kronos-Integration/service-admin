...
class ServiceAdmin extends ServiceKOA {
	...
	_start() {
		return super._start().then(() => manager.loadFlowFromFile(path.join(__dirname, '..', 'admin.flow')));
	}
}
...
module.exports.registerWithManager = manager => manager.registerServiceFactory(ServiceAdmin);
