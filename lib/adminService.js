...
module.exports.registerWithManager = function (manager) {
	manager.registerModules(['kronos-flow', 'kronos-adapter-inbound-http', 'kronos-http-routing-step',
	...
	const adminService = manager.serviceDeclare('koa-service', {
		...
		autostart: true,
		_start() {
			return koaService.start().then(service => manager.loadFlowFromFile(path.join(__dirname, '..', 'admin.flow'))
				.then(
					flow => flow.start()
				));
		}
